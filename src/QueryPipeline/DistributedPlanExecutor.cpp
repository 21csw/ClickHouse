...
protected:
    ...
    private:
        ...
        void checkStatusFunc(const String & stage_name, const RunningTaskInfo & task)
        {
            ...
            /// Forward worker log lines to the initiator's send_logs_level stream. Push before
            /// any status-derived throw below, so a failed task's exception context reaches the
            /// client's log stream before the query is failed.
