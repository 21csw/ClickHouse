...
DistributedQueryTaskStatus getTaskStatus(const String & endpoint_uri, const String & task_id, UInt32 wait_for_ms, const ContextPtr & context, bool for_cleanup)
{
    ...
    uri.addQueryParameter("status_version", toString(DBMS_TCP_PROTOCOL_VERSION));
    ...
    /// Old workers do not send the version header and answer in the legacy format.
    ...
    for (const auto & header : in->getResponseHeaders())
    {
        ...
        if (name_and_value.at(0).safeGet<String>() == "X-ClickHouse-Task-Status-Version")
            response_version = std::min<UInt64>(parse<UInt64>(name_and_value.at(1).safeGet<String>()), DBMS_TCP_PROTOCOL_VERSION);
    }
    ...
    if (result.logs.rows() != 0)
        LOG_TRACE(getLogger("StatelessWorkerClient"), "Received {} log rows for task {}", result.logs.rows(), task_id);
