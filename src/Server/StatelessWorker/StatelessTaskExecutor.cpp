...
StatelessTaskExecutor::Result StatelessTaskExecutor::startTask(const String & unique_task_id, const DistributedQueryTaskDescription & task_description, const String & unique_temp_file_path)
{
    ...
    /// Collect this task's logs for forwarding to the coordinator when the initiator asked
    /// for them (`send_logs_level` arrives via the forwarded settings changes). Mirrors what
    /// TCPHandler does for a query with `send_logs_level`.
        ...
        logs_queue = task_state->logs_queue, client_logs_level]() mutable
    {
        ...
        /// `getStatus` treats a ready future as a terminal state, the coordinator stops
        /// polling right after it, and the final logs drain (see `drainLogs`) is complete
        /// only if every log write happens-before `set_value`. The `QueryScope` destructor
        /// still logs (peak memory usage), so it must run before the promise is set.
        String error_message;
        try
        {
            /// QueryScope creation can throw (e.g. the worker is at its query limit); the outer
            /// try ensures any failure still completes the promise with the error rather than
            /// leaving it unfulfilled (which would make get_status hang or throw).
            auto query_scope = QueryScope::create(query_context);
            //TODO does this mean that threads anre not routing lgos ogs locally anymore ].
            /// Attach to the thread group so every thread of this task (including pipeline
            /// threads) routes its log lines into the queue via [
            if (logs_queue)
...
/// Pop everything currently in the queue into one block (same concatenation as
/// TCPHandler::sendLogs). Non-blocking: tryPop never waits for producers.
...
StatelessTaskExecutor::TaskStatus StatelessTaskExecutor::getStatus(const String & task_id, UInt64 wait_milliseconds)
{
    ...
    /// Drain only after the future is ready: every log write of the task happens-before
    /// `set_value` (see the task lambda in `startTask`), so this final drain is complete
    /// and the coordinator may stop polling after this terminal answer.
